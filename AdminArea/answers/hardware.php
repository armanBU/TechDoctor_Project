...
?>
 ...
 <div class="qa">
	...
	<p id="userm"><?php echo $name; ?><span>'s Query</span></p>
 	...
 	<p id="ans">উত্তর/ANS : <span><?php echo $answer; ?><a href="hardwareanswer.php?id=<?php echo $row['id']; ?>">more.</a></span></p>

 	<div class="button"><a href="hardwareanswer.php?id=<?php echo $row['id']; ?>"><p>View All</p></a></div>
 	</div>		
 	</form>
 </div>
...
<!--PAGINATION-->
<?php 
$res1=mysqli_query($db,"SELECT * FROM hardware") ;
...
<div class="center">
  ...
  <a href="hardware.php?page=<?php if($_GET['page']>1){echo ($_GET['page']-1);}else {echo "1";}?>">&laquo;</a>
    <?php
   for($i=1;$i<=$a;$i++){
        if($page==$i){
        	?>
  <a class="active" href="hardware.php?page=<?php echo $i;?>"><p><?php echo $i." "; ?></p></a>
        	<?php
        }
        else{
        ?>

        <a href="hardware.php?page=<?php echo $i;?>" ><?php echo $i." "; ?></a>
          <?php
    }
   }
   ?>
  <a href="hardware.php?page=<?php if($_GET['page']<$a){echo ($_GET['page']+1);}else {echo $a;}?>">&raquo;</a>
  </div>
</div>
